...
if backend_url:
    ALLOWED_HOSTS.append(backend_url.replace('https://', '').replace('http://', '').strip('/'))
if render_host:
...
if (use_prod_db or not DEBUG) and database_url:
    DATABASES = {
    ...
    DATABASES = {
        'default': {
            ...
            'NAME': config('DB_NAME', default='fitness_zone_db'),
